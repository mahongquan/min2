...
myglobal.webviews = {
  ...
  add: function(tabId, url) {
    ...
    electron.ipcRenderer.send('createView', {
      ...
      events: myglobal.webviews.events.map(e => e.event).filter((i, idx, arr) => arr.indexOf(i) === idx)
    });
  ...
  callAsync: function (id, method, argsOrCallback, callback) {
    ...
    if (cb) {
      ...
      myglobal.webviews.asyncCallbacks[callId] = cb
    }
    electron.ipcRenderer.send('callViewMethod', { id: id, callId: callId, method: method, args: args })
  }
};
