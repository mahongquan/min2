...
var urlParser = {
    ...
    if (url.startsWith('min:')) {
      try {
        ...
        if (/^[a-zA-Z]+$/.test(path)) {
          ...
          return urlParser.getFileURL(window.globalArgs["home"] + '/pages/' + path + '/index.html' + urlObj.search)
        }
      } catch (e) {}
    }
  ...
  isInternalURL: function (url) {
    // console.log(d2);
    var u1=urlParser.getFileURL(window.globalArgs["home"]);//E:\multi\min2\js\util
    var res=url.startsWith(u1);
    // console.log(res);
    return res;
  },
